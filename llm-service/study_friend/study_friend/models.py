...
def query(engine, model, processor, config, prompt, images, temperature = 0.1, max_tokens = 999, verbose = False):
    ...
    match engine:
        case Engine.Transformers:
           result = query_transformers(model, processor, config, prompt, images, temperature, max_tokens, device, verbose)
        case Engine.MLX_VLM:
            result = query_mlx(model, processor, config, prompt, images, temperature, max_tokens, verbose)
    return result[0] if isinstance(result, (list, tuple)) else result # grab only the result, not the metadata
