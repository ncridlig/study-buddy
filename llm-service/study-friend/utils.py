...
def get_parent_model_path(model_path):
    """
    Returns the parent model path of a given model path.

    Args:
        model_path (str): The path of the model to get the parent path of.

    Returns:
        str: The parent model path if found, else the original model path.
    """
    model_finetunigs = get_string_variants(DEFAULT_MODEL_FINETUNING_NAMES, "|")
    pattern = DEFAULT_MODEL_REGEX.format(model_finetunigs=model_finetunigs)
    ...
    if match:
        ...
        print(f"Could not find parent model of {model_path}, match found: {match}.\n"
              f"Returning itself as parent")
        return model_path
